...
static void AddTriangle(Vertex *vertices, U32 &index, 
                        float p1x, float p1y, float p1z, float u1, float v1,
                        float p2x, float p2y, float p2z, float u2, float v2,
                        float p3x, float p3y, float p3z, float u3, float v3,
...
static void AddQuad(Vertex *vertices, U32 &index,
                    float p1x, float p1y, float p1z, float u1, float v1, // Bottom-left
...
MeshData CreateSphere(Arena *arena, float radius, int sectors, int stacks)
{
    ...
    for (int i = 0; i < stacks; ++i)
    {
        ...
        for (int j = 0; j < sectors; ++j)
        {
            ...
            // p1
            float px1 = x0 * zr0; float py1 = y0 * zr0; float pz1 = z0;
            float nx1 = px1/radius; float ny1 = py1/radius; float nz1 = pz1/radius;
            // p2
            float px2 = x1 * zr0; float py2 = y1 * zr0; float pz2 = z0;
            float nx2 = px2/radius; float ny2 = py2/radius; float nz2 = pz2/radius;
            // p3
            float px3 = x1 * zr1; float py3 = y1 * zr1; float pz3 = z1;
            float nx3 = px3/radius; float ny3 = py3/radius; float nz3 = pz3/radius;
            // p4
...
MeshData CreateTorus(Arena *arena, float main_radius, float tube_radius, int main_segments, int tube_segments)
{
    MeshData mesh;
    ...
    for (int i = 0; i < main_segments; ++i)
    {
        ...
        for (int j = 0; j < tube_segments; ++j)
        {
            ...
            auto calc_pos = [=](float th, float ph, float& px, float& py, float& pz, float& nx, float& ny, float& nz) {
...
MeshData CreateSquarePyramid(Arena *arena, float base_size, float height)
{
    ...
    // Front
    AddTriangle(mesh.vertices, idx, -hb, -hh, hb, 0, 1, hb, -hh, hb, 1, 1, 0, hh, 0, 0.5f, 0, 0, 1, 1);
    // Back
    AddTriangle(mesh.vertices, idx, hb, -hh, -hb, 0, 1, -hb, -hh, -hb, 1, 1, 0, hh, 0, 0.5f, 0, 0, 1, -1);
    // Left
    AddTriangle(mesh.vertices, idx, -hb, -hh, -hb, 0, 1, -hb, -hh, hb, 1, 1, 0, hh, 0, 0.5f, 0, -1, 1, 0);
    // Right
    AddTriangle(mesh.vertices, idx, hb, -hh, hb, 0, 1, hb, -hh, -hb, 1, 1, 0, hh, 0, 0.5f, 0, 1, 1, 0);
...
MeshData CreateTriangularPrism(Arena *arena, float width, float height, float depth)
{
    MeshData mesh;
    mesh.vertex_count = 24; // 2 bases (2 tris) + 3 rectangular sides (6 tris) -> 8 tris -> 24 verts
    mesh.vertices = PushArray(arena, Vertex, mesh.vertex_count);
    U32 idx = 0;
    ...
    // Front triangle
    AddTriangle(mesh.vertices, idx, -hw, -hh, hd, 0, 1, hw, -hh, hd, 1, 1, 0, hh, hd, 0.5f, 0, 0, 0, 1);
    // Back triangle
    AddTriangle(mesh.vertices, idx, hw, -hh, -hd, 0, 1, -hw, -hh, -hd, 1, 1, 0, hh, -hd, 0.5f, 0, 0, 0, -1);
